...
{
    if(*uriLength + strlen(asciiDir1) + wcslen(asciiDir2) + 2 > MAX_URI_LENGTH || strlen(asciiDir1) >= 64 || wcslen(asciiDir2) >= 64)
    {
    ...
    extLength = sprintf_s(uri + *uriLength, 64, "%s%s", asciiDir1, TTD_HOST_PATH_SEP);
    ...
    *uriLength += extLength;
    ...
    extLength = sprintf_s(uri + *uriLength, 64, "%s%s", realAsciiDir2, TTD_HOST_PATH_SEP);
    if(extLength == -1 || MAX_URI_LENGTH < *uriLength + extLength)
    {
