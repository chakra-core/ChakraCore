...
# determine overall build status by aggregating the results of all flavors. "passed" until "failed" is encountered.
$overallBuildStatus = "passed"

$files = Get-ChildItem -Path $rootPath "*.json" -Recurse | ? { ($_.Name -ne "change.json") -and ($_.Name -ne "build.json") } | % { $_.FullName }
