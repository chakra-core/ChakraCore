...
namespace UnifiedRegex
{
    ...
    struct CharSetNode : protected Chars<wchar16>
    {
        ...
        virtual void ToEquivClassW(ArenaAllocator* allocator, uint level, uint base, uint& tblidx, CharSet<wchar16>& result) const = 0;
    ...
    public:
        ...
        void ToEquivClassW(ArenaAllocator* allocator, uint level, uint base, uint& tblidx, CharSet<wchar16>& result) const override;
    ...
    template <>
    class CharSet<wchar16> : private Chars<wchar16>
    {
    public:
    ...
    public:
        ...
        inline wchar16 Singleton() const
        {
            Assert(IsSingleton());
    ...
    private:
        ...
        CharSet<wchar16> characterPlanes [NumberOfPlanes];

        // Takes a character, and returns the index of the CharSet<wchar16> that holds it.
        inline int CharToIndex(Char c) const
        {
            Assert(c <= Chars<codepoint_t>::MaxUChar);
            return (int)(CTU(c) / (Chars<wchar16>::MaxUChar + 1));
        }

        // Takes a character, and removes the offset to make it < 0x10000
        inline wchar16 RemoveOffset(Char c) const
        {
            Assert(c <= Chars<codepoint_t>::MaxUChar);
            return (wchar16)(CTU(c) % 0x10000);
        }

        // Takes a character, and removes the offset to make it < 0x10000
        inline Char AddOffset(wchar16 c, int index) const
        {
    ...
    public:
        ...
        void Clear(ArenaAllocator* allocator);
        void CloneFrom(ArenaAllocator* allocator, const CharSet<Char>& other);
        void CloneSimpleCharsTo(ArenaAllocator* allocator, CharSet<wchar16>& other) const;

        inline void CloneNonSurrogateCodeUnitsTo(ArenaAllocator* allocator, CharSet<wchar16>& other)
        {
        ...
        inline void CloneSurrogateCodeUnitsTo(ArenaAllocator* allocator, CharSet<wchar16>& other)
        {
            Assert(this->SimpleCharCount() > 0);
        ...
        {
            wchar16 outLower = 0xFFFF, ignore = 0x0;
        ...
        void SetNotRanges(ArenaAllocator* allocator, int numSortedPairs, const Char* sortedPairs);
        void UnionInPlace(ArenaAllocator* allocator, const  CharSet<Char>& other);
        void UnionInPlace(ArenaAllocator* allocator, const  CharSet<wchar16>& other);
        _Success_(return) bool GetNextRange(Char searchCharStart, _Out_ Char *outLowerChar, _Out_ Char *outHigherChar);
        ...
        inline wchar16 SimpleCharSingleton() const
        {
        ...
        void ToSimpleComplement(ArenaAllocator* allocator, CharSet<wchar16>& result);
        void ToEquivClass(ArenaAllocator* allocator, CharSet<Char>& result);
    ...
    template <>
    class RuntimeCharSet<wchar16> : private Chars<wchar16>
    {
    private:
        // Trie for remaining characters. Pointer value will be 0 or >> MaxCompact.
        CharSetNode* root;
    ...
    typedef CharSet<wchar16> UnicodeCharSet;
    typedef RuntimeCharSet<wchar16> UnicodeRuntimeCharSet;
}
