...
CmdLineArgsParser::ParseString(__inout_ecount(ceBuffer) LPWSTR buffer, size_t ceBuffer, bool fTreatColonAsSeparator)
{
    ...
    if('"' == CurChar())
    {
        ...
        while('"' != CurChar())
        {
            if(0 == CurChar())
            {
                throw Exception(CH_WSTR("Unmatched quote"));
            }
            ...
            if (len >= ceBuffer - 1)
            {
                throw Exception(CH_WSTR("String token too large to parse"));
            }
    ...
    if(0 == len)
    {
        throw Exception(CH_WSTR("String Token Expected"));
    }
...
CmdLineArgsParser::ParseInteger()
{
    ...
    if(!IsDigit())
    {
        throw Exception(CH_WSTR("Integer Expected"));
    }
    ...
    while(IsDigit() || (base == 16 && IsHexDigit()))
    {
        ...
        if(result < 0)
        {
            ...
            throw Exception(CH_WSTR("Integer too large to parse"));
        }
...
CmdLineArgsParser::ParseRange(Js::Range *pRange)
{
    ...
    case '-':
        ...
        {
            throw Exception(CH_WSTR("Left source index must be smaller than the Right source Index"));
        }
        if ((r1.sourceContextId == r2.sourceContextId) &&
            (r1.functionId > r2.functionId))
        {
            throw Exception(CH_WSTR("Left functionId must be smaller than the Right functionId when Source file is the same"));
        }
        ...
        default:
            throw Exception(CH_WSTR("Unexpected character while parsing Range"));
        }
        break;
...
CmdLineArgsParser::ParseNumberRange(Js::NumberRange *pRange)
{
    ...
    case '-':
        ...
        if (start > end)
        {
            throw Exception(CH_WSTR("Range start must be less than range end"));
        }
...
CmdLineArgsParser::ParseBoolean()
{
    if (CurChar() == ':')
    {
        throw Exception(CH_WSTR("':' not expected with a boolean flag"));
    }
    else if (CurChar() != '-' && CurChar() != ' ' && CurChar() != 0)
    {
        throw Exception(CH_WSTR("Invalid character after boolean flag"));
    }
    else
    {
        return (CurChar() != '-');
...
CmdLineArgsParser::GetCurrentString()
{
    ...
    case 0:
        ...
        return nullptr;
    default:
        throw Exception(CH_WSTR("Expected ':'"));
    }
}
...
CmdLineArgsParser::ParseFlag()
{
    ...
    if(InvalidFlag == flag)
    {
        ...
        throw Exception(CH_WSTR("Invalid Flag"));
    }
...
int CmdLineArgsParser::Parse(__in LPWSTR oneArg) throw()
{
    ...
    try
    {
        ...
        default:
            ...
            {
                throw Exception(CH_WSTR("Duplicate filename entry"));
            }
