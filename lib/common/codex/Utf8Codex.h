...
namespace utf8
{
    ...
    //                     UCS-2. This is technically incorrect but usually harmless. This file assumes wchar16 *
    ...
    //                     RFC-2781. Two wchar16 values are used to encode one UCS code point.
    ...
    inline size_t EncodedSize(wchar16 ch)
    {
    ...
    wchar16 DecodeTail(wchar16 c1, LPCUTF8& ptr, LPCUTF8 end, DecodeOptions& options);
    ...
    inline wchar16 Decode(LPCUTF8& ptr, LPCUTF8 end, DecodeOptions& options)
    {
        if (ptr >= end) return 0;
        utf8char_t c1 = *ptr++;
        if (c1 < 0x80) return static_cast<wchar16>(c1);
    ...
    LPUTF8 EncodeFull(wchar16 ch, __out_ecount(3) LPUTF8 ptr);

    // Encode ch into a UTF8 sequence ignoring surrogate pairs (which are encoded as two
    // separate code points).
    inline LPUTF8 Encode(wchar16 ch, __out_ecount(3) LPUTF8 ptr)
    {
        if (ch < 0x80)
        {
    ...
    void DecodeInto(__out_ecount_full(cch) wchar16 *buffer, LPCUTF8 ptr, size_t cch, DecodeOptions options = doDefault);

    // Provided for dual-mode templates
    inline void DecodeInto(__out_ecount_full(cch) wchar16 *buffer, const wchar16 *ptr, size_t cch, DecodeOptions /* options */ = doDefault)
    {
        memcpy_s(buffer, cch * sizeof(wchar16), ptr, cch * sizeof(wchar16));
    }

    // Like DecodeInto but ensures buffer ends with a NULL at buffer[cch].
    void DecodeIntoAndNullTerminate(__out_ecount(cch+1) __nullterminated wchar16 *buffer, LPCUTF8 ptr, size_t cch, DecodeOptions options = doDefault);
    ...
    size_t DecodeUnitsInto(_Out_writes_(pbEnd - pbUtf8) wchar16 *buffer, LPCUTF8& pbUtf8, LPCUTF8 pbEnd, DecodeOptions options = doDefault);
    ...
    size_t DecodeUnitsIntoAndNullTerminate(__out_ecount(pbEnd - pbUtf8 + 1) __nullterminated wchar16 *buffer, LPCUTF8& pbUtf8, LPCUTF8 pbEnd, DecodeOptions options = doDefault);
    ...
    __range(0, cch * 3)
    size_t EncodeInto(__out_ecount(cch * 3) LPUTF8 buffer, __in_ecount(cch) const wchar16 *source, charcount_t cch);

    // Like EncodeInto but ensures that buffer[return value] == 0.
    __range(0, cch * 3)
    size_t EncodeIntoAndNullTerminate(__out_ecount(cch * 3 + 1) utf8char_t *buffer, __in_ecount(cch) const wchar16 *source, charcount_t cch);
