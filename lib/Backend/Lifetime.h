...
public:
    ...
    Lifetime(JitArenaAllocator * alloc, StackSym *sym, RegNum reg, uint32 start, uint32 end, Func *func)
        ...
        isNotInt(0), cantSpill(false), dontAllocate(false), isSecondChanceAllocated(false), isCheapSpill(false), spillStackSlot(NULL),
...
public:
    ...
    bool IsSimd128()
    {
        // Make sure isNotInt is big enough for all the bits in the struct
        CompileAssert((sizeof(Lifetime) - offsetof(Lifetime, isNotInt)) == sizeof(isNotInt));
        return !!(isNotInt & ~1 /*remove isFloat bit*/);
    }
