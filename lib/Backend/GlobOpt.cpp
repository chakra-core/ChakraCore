...
GlobOpt::MergePredBlocksValueMaps(BasicBlock *block)
{
    ...
    if (block->isLoopHeader)
    {
        ...
        if (func->HasSIMDOps())
        {
            SetSimd128LoopHeaderValues(block, symTable, symToValueMap);
        }

        tempBv->ClearAll();
    }

    if (!block->isLoopHeader && func->HasSIMDOps())
    {
        // After merging Simd128 liveness bits from preds, we might have decided to keep the type-spec sym.
        // This however might not be reflected on the merged ValueTypes. This is because when merging a LikelySimd128F4 and DefiniteSimd128F4, 
        // we end up with LikelySimd128F4 (ORing of value bits). Go over the merged liveness bits and set the merge ValueType to definite based on merged liveness bits.
        SetSimd128MergedValues(block);
    }
    ...
    if (val)
    {
            ...
            IsLive(sym, &rootLoopPrePass->landingPad->globOptData) && !isAsmJSFunc) // no typespec in asmjs and hence skipping this
        {
            ...
            if(landingPadValue && val->GetValueNumber() == landingPadValue->GetValueNumber())
            {
                ...
                if (func->HasSIMDOps() && Js::IsSimd128Opcode(instr->m_opcode))
                {
    ...
    default:
        {
            ...
            case Js::OpCode::ArgOut_A_InlineBuiltIn:
                ...
                if (instr->GetSrc1()->IsRegOpnd())
                {
                    ...
                    else if (func->HasSIMDOps() && this->IsSimd128TypeSpecialized(sym, this->currentBlock))
                    {
                        // SIMD_JS
...
GlobOpt::ToVar(IR::Instr *instr, IR::RegOpnd *regOpnd, BasicBlock *block, Value *value, bool needsUpdate)
{
    ...
    {
        // SIMD_JS
        Assert(func->HasSIMDOps() && IsSimd128TypeSpecialized(varSym, &block->globOptData));
...
GlobOpt::ToTypeSpecUse(IR::Instr *instr, IR::Opnd *opnd, BasicBlock *block, Value *val, IR::IndirOpnd *indir, IRType toType, IR::BailOutKind bailOutKind, bool lossy, IR::Instr *insertBeforeInstr)
{
        ...
        ))
    {
        ...
        {
            // SIMD_JS
            Assert(func->HasSIMDOps() && IRType_IsSimd128(toType));
...
{
    return  data->liveSimd128F4Syms && 
            (IsSimd128TypeSpecialized<TySimd128F4>(sym, data) ||
...
#define SIMD_HOIST(_NAME_,_TAG_) \
                    ...
                    else SIMD_EXPAND_W_NAME_CHECK(SIMD_HOIST, func->HasSIMDOps())
                    {
                        Assert(UNREACHED);
                    }
#undef SIMD_HOIST
...
GlobOpt::PreOptPeep(IR::Instr *instr)
{
    if (OpCodeAttr::HasDeadFallThrough(instr->m_opcode))
    {
        switch (instr->m_opcode)
        {
            ...
            {
                // Nothing to do here. FallThru code already removed in GlobOptSimd128
                return instr;
            }
...
GlobOpt::MergeSimd128Liveness(GlobOptBlockData * fromData, GlobOptBlockData * toData, BVSparse<JitArenaAllocator> * fromLiveSimd128Syms, BVSparse<JitArenaAllocator> * toLiveSimd128Syms, BVSparse<JitArenaAllocator> * simdSymsToVar, ObjectType simdType, bool isLoopBackEdge)
{
    ...
    //     (1) We are not in jitLoopBody. Introducing a FromVar for compensation extends bytecode syms lifetime. If the value
    //       is actually backward dead, and we enter the loop-body after bailing out from SimpleJit, the value will not be restored by the SimpleJit
    //       bailout code, and we might execute FromVar on a NULL value.
    //       Consider this example:
    //             x = f4op();
    //             y = f4op(x, x);
    //             // x is backwards dead here and won't be restored on simpleJit bailout to interpreter
    //             while(cond1)
    //             {
    //                 if (cond2)
    //                 {
    ...
    //                     y = f4op(x, x);
    //                 }
    //                 // In FullJit, on merging Values, x is forward live as F4 on one edge, and as Var on another, we will insert FromVar x here
    //                 // but if cond2 is false, we will do FromVar NULL
    //                 x = f4op();
    //             }
    //     (2) Value was never Undefined/Null. Avoid unboxing of possibly uninitialized values.
    //     (3) Not loop back-edge. To keep unboxed value, the value has to be used-before def in the loop-body. This is done
...
void
GlobOpt::CompensateSimd128OnBackEdge(BasicBlock * block, BVSparse<JitArenaAllocator> * loopSimd128SymsOnEntry, BVSparse<JitArenaAllocator> * liveSimd128Syms, BVSparse<JitArenaAllocator> * liveOnBackEdge, IR::BailOutKind bailOutKind, IRType type)
{
...
void
GlobOpt::CompensateSimd128OnLandingPad(BasicBlock * block, BVSparse<JitArenaAllocator> * loopSimd128SymsOnEntry, BVSparse<JitArenaAllocator> * landingPadLiveSimd128Syms, BVSparse<JitArenaAllocator> * blockLiveSimd128Syms, BVSparse<JitArenaAllocator> * liveOnBackEdge, IR::BailOutKind bailOutKind, IRType type)
{
...
void
GlobOpt::SetSimd128MergedValues(BasicBlock * mergedBlock)
{
    SymTable *const symTable = func->m_symTable;
    GlobHashTable *const symToValueMap = mergedBlock->globOptData.symToValueMap;
    BVSparse<JitArenaAllocator> * bv;

#define SET_VALUE_INFO(_NAME_,_TAG_)\
    bv = mergedBlock->globOptData.liveSimd128##_TAG_##Syms;\
    FOREACH_BITSET_IN_SPARSEBV(id, bv)\
    {\
        StackSym *const varSym = symTable->FindStackSym(id);\
        Assert(varSym);\
        Value *const value = FindValue(symToValueMap, varSym);\
        ValueInfo *const valueInfo = value ? value->GetValueInfo() : nullptr;\
        if (valueInfo && !valueInfo->IsSimd128##_NAME_##())\
        {\
            Assert(valueInfo->IsLikelySimd128##_NAME_##());\
            ChangeValueInfo(mergedBlock, value, valueInfo->SpecializeToSimd128##_TAG_##(alloc));\
        }\
    } NEXT_BITSET_IN_SPARSEBV;
...
void
GlobOpt::SetSimd128LoopHeaderValues(BasicBlock * loopHeader, SymTable *const symTable, GlobHashTable *const symToValueMap)
{
    BVSparse<JitArenaAllocator> tempBv2(this->tempAlloc);
    ...
    FOREACH_BITSET_IN_SPARSEBV(id, tempBv)
    {
        ...
        // if the liveness is set for a sym in the header block, and it has no type-spec sym that is live in the landing-pad,
        // then this means that sym was introduced because of hoisting or live on backedged and used before defined (see AdjustSimd128LivenessOnLoopHeader)
        // which means it's value is not set to definite in the header block, so set it here. 
#define SET_VALUE_INFO(_NAME_,_TAG_) \
