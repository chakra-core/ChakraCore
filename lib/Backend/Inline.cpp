    ...
    const Js::ProfileId profileId, bool* pIsInlined, IR::Instr * inlineeDefInstr, uint recursiveInlineDepth)
{
    ...
    *pIsInlined = true;
    ...
    if (callInstr->m_opcode == Js::OpCode::NewScObject || callInstr->m_opcode == Js::OpCode::NewScObjArray)
    {
        ...
        if (callInstr->IsProfiledInstr())
        {
            Js::ProfileId profiledCallSiteId = static_cast<Js::ProfileId>(callInstr->AsProfiledInstr()->u.profileId);
            const JITTimeConstructorCache*  constructorCache = callInstr->m_func->GetConstructorCache(profiledCallSiteId);
            bool skipNewScObj = constructorCache != nullptr && constructorCache->SkipNewScObject();
            if (!skipNewScObj)
            {
                // Convert GenCtorObj into a profiled instr.
                IR::Instr* originalGenCtorObjInstr = callInstr->GetGenCtorInstr();
                Assert(!originalGenCtorObjInstr->HasBailOutInfo());
                IR::ProfiledInstr* profiledGenCtorObjInstr = IR::ProfiledInstr::New(
                    Js::OpCode::GenCtorObj, originalGenCtorObjInstr->UnlinkDst(),
                    originalGenCtorObjInstr->UnlinkSrc1(), originalGenCtorObjInstr->m_func);
                profiledGenCtorObjInstr->SetByteCodeOffset(originalGenCtorObjInstr);
                profiledGenCtorObjInstr->AsProfiledInstr()->u.profileId = profiledCallSiteId;
                originalGenCtorObjInstr->InsertAfter(profiledGenCtorObjInstr);
                originalGenCtorObjInstr->Remove();
            }
        }
