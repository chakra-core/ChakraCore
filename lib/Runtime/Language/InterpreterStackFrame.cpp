...
namespace Js
{
    ...
    )
    {
        ...
        bool doJITLoopBody =
            ...
            !this->executeFunction->GetUtf8SourceInfo()->IsUtf8SourceInfoInDebugMode();
#else
...
#else
        if (newInstance->m_functionBody->GetUtf8SourceInfo()->IsUtf8SourceInfoInDebugMode())
        {
...
#pragma optimize("", on)
    ...
    Var InterpreterStackFrame::InterpreterHelper(ScriptFunction* function, ArgumentReader args, void* returnAddress, void* addressOfReturnAddress, const bool isAsmJs)
    {
...
#ifdef ENABLE_DEBUG_CONFIG_OPTIONS
        if (!isAsmJs && executeFunction->GetUtf8SourceInfo()->IsUtf8SourceInfoInDebugMode() != functionScriptContext->IsScriptContextInDebugMode()) // debug mode mismatch
        {
            if (executeFunction->GetUtf8SourceInfo()->GetIsLibraryCode())
            {
                Assert(!executeFunction->GetUtf8SourceInfo()->IsUtf8SourceInfoInDebugMode()); // Library script byteCode is never in debug mode
            }
            else
            {
        ...
        const bool doProfile = executeFunction->GetInterpreterExecutionMode(false) == ExecutionMode::ProfilingInterpreter ||
                               executeFunction->GetUtf8SourceInfo()->IsUtf8SourceInfoInDebugMode() && DynamicProfileInfo::IsEnabled(executeFunction);
        if (doProfile)
        {
#if !DYNAMIC_INTERPRETER_THUNK
        ...
        {
            if (!isAsmJs && executeFunction->GetUtf8SourceInfo()->IsUtf8SourceInfoInDebugMode())
            {
#if DYNAMIC_INTERPRETER_THUNK
...
// Do MTJRC/MAIC:0 check
    ...
    Var InterpreterStackFrame::ProcessLinkFailedAsmJsModule()
    {
        ...
        const bool doProfile =
            ...
            funcObj->GetFunctionBody()->GetUtf8SourceInfo()->IsUtf8SourceInfoInDebugMode() && DynamicProfileInfo::IsEnabled(funcObj->GetFunctionBody());
...
const byte * InterpreterStackFrame::OP_ProfiledLoopBodyStart(const byte * ip)
{
    ...
    void InterpreterStackFrame::OP_TryCatch(const unaligned OpLayoutBr* playout)
    {
        ...
        try
        {
            ...
            if (this->GetFunctionBody()->GetUtf8SourceInfo()->IsUtf8SourceInfoInDebugMode())
            {
    ...
    void InterpreterStackFrame::ProcessTryFinally(const byte* ip, Js::JumpOffset jumpOffset, Js::RegSlot regException, Js::RegSlot regOffset, bool hasYield)
    {
        ...
        if (pExceptionObject && this->GetFunctionBody()->GetUtf8SourceInfo()->IsUtf8SourceInfoInDebugMode() &&
