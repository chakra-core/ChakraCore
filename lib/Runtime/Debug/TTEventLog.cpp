...
namespace TTD
{
    ...
    void EventLog::LoadLastSourceLineInfo(TTInnerLoopLastStatementInfo& lsi, TTD::TTDebuggerSourceLocation& dsl) const
    {
        ...
        if(iter.IsValid() && iter.Current()->EventKind == NSLogEvents::EventKind::TTDInnerLoopLogWriteTag)
        {
            ...
            lsi.SetLastLine(ilevt->ETime, ilevt->FTime, ilevt->LTime, ilevt->Line, ilevt->Column);
            dsl.SetLocationFullRaw(ilevt->SourceScriptLogId, ilevt->ETime, ilevt->FTime, ilevt->LTime, ilevt->TopLevelBodyId, ilevt->FunctionLine, ilevt->FunctionColumn, ilevt->Line, ilevt->Column);
        }
    }
    ...
    void EventLog::InnerLoopEmitLog(const TTDebuggerSourceLocation& writeLocation, const char* emitUri, size_t emitUriLength)
    {
        ...
        evt->ETime = writeLocation.GetRootEventTime();
        evt->FTime = writeLocation.GetFunctionTime();
        evt->LTime = writeLocation.GetLoopTime();
