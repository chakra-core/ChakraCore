...
(function (EngineInterface, InitType) {
    ...
    function InitializeLangTagREs() {
        ...
        const irregular = "(?:en-GB-oed|i-ami|i-bnn|i-default|i-enochian|i-hak|i-klingon|i-lux|i-mingo" +
            "|i-navajo|i-pwn|i-tao|i-tay|i-tsu|sgn-BE-FR|sgn-BE-NL|sgn-CH-DE)";
        const grandfathered = `(?:${regular}|${irregular})`;

        const privateuse = `(?:x(?:-${alphanum}{1,8})+)`;                   // privateuse    = "x" 1*("-" (1*8alphanum))
        const singleton = `(?:${DIGIT}|[A-WY-Z])`;                          // singleton    ~= alphanum except for 'x'          ; (paraphrased)
        const extension = `(?:${singleton}(?:-${alphanum}{2,8})+)`;         // extension     = singleton 1*("-" (2*8alphanum))
        const variant = `(?:${alphanum}{5,8}|${DIGIT}${alphanum}{3})`;      // variant       = 5*8alphanum / (DIGIT 3alphanum)
        const region = `(?:${ALPHA}{2}|${DIGIT}{3})`;                       // region        = 2ALPHA / 3DIGIT

        const script = `(?:${ALPHA}{4})`;                                   // script        = 4ALPHA
        const extlang = `(?:${ALPHA}{3}(?:-${ALPHA}{3}){0,2})`;             // extlang       = 3ALPHA *2("-" 3ALPHA)

        const language = '(?:'     +                                        // language      =
            `${ALPHA}{2,3}`        +                                        //                 2*3ALPHA            ; shortest ISO 639 code
                `(?:-${extlang})?` +                                        //                 ["-" extlang]       ; sometimes followed by extended language subtags
        ...
        const LANG_TAG_BASE = `(${language})`     +                         // langtag       = language
                              `(?:-(${script}))?`   +                       //                 ["-" script]
                              `(?:-(${region}))?`   ;                       //                 ["-" region]
        const LANG_TAG_EXT  = `(?:-${variant})*`    +                       //                 *("-" variant)
                              `(?:-${extension})*`  +                       //                 *("-" extension)
                              `(?:-${privateuse})?` ;                       //                 ["-" privateuse]
        const langtag       = `${LANG_TAG_BASE}${LANG_TAG_EXT}`;

        const LANG_TAG      = `${langtag}|${privateuse}|${grandfathered}`;  // Language-Tag  = ...

        LANG_TAG_BASE_RE    = new RegExp(LANG_TAG_BASE, 'i');
        LANG_TAG_EXT_RE     = new RegExp(LANG_TAG_EXT,  'i');
        LANG_TAG_RE         = new RegExp(LANG_TAG,      'i');
    }
