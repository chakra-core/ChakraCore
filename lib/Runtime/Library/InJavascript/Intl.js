...
(function (EngineInterface, InitType) {
    ...
    const _ = {
        ...
        stringIndexOf(str, el, from) { return String.prototype.indexOf.call(str, el, from); },
    ...
    var DateTimeFormat = (function () {
        if ((InitType === 'Intl' || InitType === 'Date') && isPlatformUsingICU) {
            ...
            const allOptionValues = [].concat(twoDigitNumeric, narrowShortLong);
            ...
            const LookupMatcher = function (requestedLocales) {
                const result = bare();
                _.forEach(requestedLocales, function (locale) {
                    const parts = _.match(locale, LANG_TAG_RE); // TODO
            ...
            const UnicodeExtensionValue = function (extension, key) {
                ...
                let searchValue = `-${key}-`;
                const pos = _.stringIndexOf(extension, searchValue);
                if (pos !== -1) {
                    const start = pos + 4;
                    let end = start;
                    let k = start;
                ...
                searchValue = `-${key}`;
                pos = _.stringIndexOf(extension, searchValue);
                if (pos !== -1 && pos + 3 === size) {
                    return "";
                } else {
                    return undefined;
            ...
            const ResolveLocale = function (requestedLocales, options, relevantExtensionKeys, localeData) {
                ...
                _.forEach(relevantExtensionKeys, function (key) {
                    ...
                    if (r.extension) {
                        ...
                        if (requestedValue !== undefined) {
                            ...
                            } else if (_.stringIndexOf(keyLocaleData, "true") !== -1) {
                                value = "true";
                            }
                        }
                    }
            ...
            const CanonicalizeTimeZoneName = function (timeZone) {
                // TODO
            ...
            const InitializeDateTimeFormat = function (dateTimeFormat, locales, options) {
                ...
                const r = ResolveLocale(requestedLocales, opt, ["nu", "ca", "hc"], localeData); // TODO
                ...
                if (tz !== undefined) {
                    ...
                    if (!platform.isValidTimeZoneName(tz)) { // TODO
                        platform.raiseOptionValueOutOfRange();
                    }
                    tz = CanonicalizeTimeZoneName(tz); // TODO
                } else {
                    tz = DefaultTimeZone(); // TODO
                }
            ...
            // ECMA402: #sec-basicformatmatcher
            const BasicFormatMatcher = function (options, formats) {
                ...
                _.forEach(formats, function (format) {
                    let score = 0;
                    _.forEach(dateTimeComponents, function (component) {
                        const optionsProp = options[component[0]];
                        const formatProp = format[component[0]];
                        if (optionsProp === undefined && formatProp !== undefined) {
                            score -= additionPenalty;
                        } else if (optionsProp !== undefined && formatProp === undefined) {
                            score -= removalPenalty;
                        } else if (optionsProp !== formatProp) {
                            const optionsPropIndex = _.arrayIndexOf(allOptionValues, optionsProp);
                            const formatPropIndex = _.arrayIndexOf(allOptionValues, formatProp);
                            const delta = _.max(_.min(formatPropIndex - optionsPropIndex, 2), -2);
                            if (delta === 2) {
            ...
            const BestFitFormatMatcher = function (options, formats) {
                return BasicFormatMatcher(options, formats);
            };

            const PartitionDateTimePattern = function (dtf, x) {
                if (!_.isFinite(x)) {
                    raise.rangeError();
                }
                ...
                // TODO(jahorto): we will need to make sure NumberFormat is initialized for this...
                const nf = new NumberFormat(locale, bare({ useGrouping: false }));
                const nf2 = new NumberFormat(locale, bare({ minimumIntegerDigits: 2, useGrouping: false }));
                const tm = ToLocaleTime(x, dtf.calendar, dtf.timeZone);
                const pattern = dtf.pattern;
                const result = [];
                let beginIndex = _.stringIndexOf(pattern, "{", 0);
                let endIndex = 0;
                let nextIndex = 0;
                let length = pattern.length;
                while (beginIndex >= 0 && beginIndex < length) {
                    endIndex = _.stringIndexOf(pattern, "}", beginIndex);
                    raise.assert(endIndex > beginIndex);
                    if (beginIndex > nextIndex) {
                        _.push(result, bare({ type: "literal", value: _.substring(pattern, nextIndex, beginIndex) }));
                    }

                    const p = _.substring(pattern, beginIndex + 1, endIndex); // exclusive, exclusive
                    const pComponentIndex = _.findIndex(dateTimeComponents, (component) => p === component[0]);
                    if (pComponentIndex !== -1) {
                        const slotName = dateTimeComponents[pComponentIndex][0];
                        const f = dtf[slotName];
                        const v = tm[slotName];
                        if (p === "year" && v <= 0) {
                            v = 1 - v;
                        } else if (p === "month") {
                            v += 1;
                        } else if (p === "hour" && (dtf.hourCycle === "h11" || dtf.hourCycle === "h12")) {
                            v %= 12;
                            if (v === 0 && dtf.hourCycle === "h12") {
                                v = 12;
                            }
                        } else if (p === "hour" && dtf.hourCycle === "h24" && v === 0) {
                            v = 24;
                        }
                        ...
                        if (f === "numeric") {
                            ...
                            fv = nf2.format(v);
                            if (fv.length > 2) {
                                fv = _.substring(fv, fv.length - 2);
                            }
                        } else if (_.arrayIndexOf(narrowShortLong, f) !== -1) {
                            // TODO: spec below
                            // let fv be a String value representing f in the desired form;
                            // the String value depends upon the implementation and the effective locale and calendar of dateTimeFormat.
                            // If p is "month", then the String value may also depend on whether dateTimeFormat has a [[Day]] internal slot.
                            // If p is "timeZoneName", then the String value may also depend on the value of the [[inDST]] field of tm.
                            // If p is "era", then the String value may also depend on whether dateTimeFormat has a [[Era]] internal slot and
                            // if the implementation does not have a localized representation of f, then use f itself.
                            fv = f;
                        }

                        _.push(result, bare({ type: p, value: fv }));
                    } else if (p === "ampm") {
                        const v = tm.hour;
                        let fv;
                        if (v > 11) {
                            fv = "post meridiam"; // TODO: localize
                        } else {
                            fv = "ante meridiam"; // TODO: localize
                        }

                        _.push(result,
                            bare({ type: "dayPeriod", value: fv }),
                            bare({ type: "literal",  value: _.substring(pattern, beginIndex, endIndex + 1) }) // inclusive, inclusive
                        );
                    }

                    nextIndex = endIndex + 1;
                    beginIndex = _.stringIndexOf(pattern, "{", nextIndex);
                }

                if (nextIndex < length) {
                    _.push(result, bare({ type: "literal", value: _.substring(pattern, nextIndex + 1, length)})); // exclusive, exclusive
                }
            ...
            const FormatDateTime = function (dtf, x) {
                ...
                return _.join(_.map(parts, (part) => part.value), "");
            };

            const FormatDateTimeToParts = function (dtf, x) {
                const parts = PartitionDateTimePattern(dtf, x);

                // REVIEW(jahorto): the spec suggests creating a new array and copying properties
                // TODO: determine if copying is necessary or if its just their conversion from record/list to object/array
                // TODO: determine if copying is necessary for cacheing purposes
                return _.map(parts, (part) => ({ type: part.type, value: part.value }));
            };

            const ToLocaleTime = function (date, calendar, timeZone) {
                // ???
                return {
                    weekday: "",
                    era: "",
            ...
            if (InitType === "Intl") {
                // TODO: does this match the spec?
                const DateTimeFormat = function (locales, options) {
                    const newTarget = new.target === undefined ? this : new.target;

                    // Use the hidden object to store data
                    let hiddenObject = platform.getHiddenObject(newTarget);
                    if (hiddenObject === undefined) {
                        platform.setHiddenObject(newTarget, bare());
                    }
