...
namespace Js
{
    ...
    JavascriptPromise* JavascriptPromise::CreateRejectedPromiseFromExceptionObject(JavascriptExceptionObject* exceptionObject, ScriptContext* scriptContext)
    {
        Var thrownObject = exceptionObject->GetThrownObject(scriptContext);
        ...
        return CreateRejectedPromise(thrownObject, scriptContext);
    }

    JavascriptPromise* JavascriptPromise::CreateRejectedPromise(Var resolution, ScriptContext* scriptContext)
    {
        Var result = CreateRejectedPromise(resolution, scriptContext, scriptContext->GetLibrary()->GetPromiseConstructor());

        // We constructed result from %Promise% so it has to be a JavascriptPromise
        return JavascriptPromise::FromVar(result);
    }

    Var JavascriptPromise::CreateRejectedPromise(Var resolution, ScriptContext* scriptContext, Var promiseConstructor)
    {
        Assert(promiseConstructor != nullptr);
    ...
    JavascriptPromise* JavascriptPromise::CreateResolvedPromise(Var resolution, ScriptContext* scriptContext)
    {
        Var result = CreateResolvedPromise(resolution, scriptContext, scriptContext->GetLibrary()->GetPromiseConstructor());
