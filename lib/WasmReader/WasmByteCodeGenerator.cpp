...
WasmBytecodeGenerator::GenerateFunction()
{
    ...
    while ((op = m_reader->ReadExpr()) != wnLIMIT)
    {
        ...
        GetRegisterSpace(exprInfo.type)->ReleaseLocation(&exprInfo);
    }
...
WasmBytecodeGenerator::EmitSetLocal()
{
    ...
    if (m_funcInfo->GetLocalCount() < localNum)
    {
    ...
    Js::OpCodeAsmJs op = GetLoadOp(local.type);
    WasmRegisterSpace * regSpace = GetRegisterSpace(local.type);
    WasmOp childOp = m_reader->ReadExpr();
    EmitInfo info = EmitExpr(childOp);
    ...
    // TODO: being conservative here, but is this tmp move required?
