...
// Helper to see the stack while debugging. To use in the Immediate window of VS
void
WasmBytecodeGenerator::PrintEvalStack() const
{
    int depth = 0;
    FOREACH_SLIST_ENTRY(evalStackScope*, scope, m_evalStack)
    {
        ++depth;
        FOREACH_SLIST_ENTRY(EmitInfo, data, scope)
        {
            switch(data.type) 
            {
            case WasmTypes::Void:
                Output::Print(_u("%*sVoid\n"), depth, "");
                break;
#define WASM_LOCALTYPE(token, name) case WasmTypes::token: Output::Print(_u("%*s" #name## ": %d\n"), depth, "", data.location); break;
#include "WasmKeywords.h"
            }
        } NEXT_SLIST_ENTRY;
    } NEXT_SLIST_ENTRY;
