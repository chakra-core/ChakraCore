...
namespace TTD
{
    ...
    public:
        ...
        void RecordTelemetryLogEvent(Js::JavascriptString* infoStringJs, bool shouldPrint, int64 optUserEventId, bool shouldBreak);

        //
        //TODO: we will want to add args from user here and compare with what we got during record to check for divegence if we are running with diagnostics
        //
        ...
        //Get the first telemetry break-event from the log and report the location info
        bool TryGetFirstTelemetryBreakLocation(TTDebuggerSourceLocation& sourceLocation) const;
#endif
